...
const AddForm = () => {
  ...
  const isAddPage = pathname.endsWith(routes.superadmin["profit-rates"].add);
  ...
  const onSubmit = async (values) => {
    ...
    await axios
      .post("/api/admin/profit-rate", encodedFormData)
      .then((response) => {
        if (response.status === 201) {
          toast.success(
            <div className="flex items-center gap-2">
              ...
              <span>{"نرخ کمیسیون جدید با موفقیت اضافه شد"}</span>
            </div>,
          );
