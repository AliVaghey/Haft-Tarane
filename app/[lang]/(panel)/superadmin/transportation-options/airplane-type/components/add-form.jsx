...
const AddForm = () => {
  ...
  const isAddPage = pathname.endsWith(
    routes.superadmin["transportation-options"]["airplane-type"].add,
  );
        ...
        if (response.status === 200) {
          toast.success(
            <div className="flex items-center gap-2">
              ...
              <span>{"نوع هواپیما با موفقیت اضافه شد"}</span>
            </div>,
          );

          router.push(
            routes.superadmin["transportation-options"]["airplane-type"].root,
          );
          router.refresh();
        }
      })
