...
const AddForm = () => {
  ...
  const isAddPage = pathname.endsWith(
    routes.superadmin["transportation-options"].bus.add,
  );
        ...
        if (response.status === 200) {
          toast.success(
            <div className="flex items-center gap-2">
              ...
              <span>{"شرکت حمل و نقل جاده ای با موفقیت اضافه شد"}</span>
            </div>,
          );

          router.push(routes.superadmin["transportation-options"].bus.root);
          router.refresh();
        }
      })
      .catch((error) => {
