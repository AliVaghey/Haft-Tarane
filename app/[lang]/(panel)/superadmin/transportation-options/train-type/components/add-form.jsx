...
const AddForm = () => {
  ...
  const isAddPage = pathname.endsWith(
    routes.superadmin["transportation-options"]["train-type"].add,
  );
        ...
        if (response.status === 200) {
          toast.success(
            <div className="flex items-center gap-2">
              ...
              <span>{"نوع قطار با موفقیت اضافه شد"}</span>
            </div>,
          );

          router.push(
            routes.superadmin["transportation-options"]["train-type"].root,
          );
          router.refresh();
        }
      })
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="mr-4 text-right">افزودن نوع قطار</DialogTitle>
        </DialogHeader>
