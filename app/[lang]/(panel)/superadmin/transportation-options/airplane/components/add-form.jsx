...
const AddForm = () => {
  ...
  const isAddPage = pathname.endsWith(
    routes.superadmin["transportation-options"].airplane.add,
  );
        ...
        if (response.status === 200) {
          toast.success(
            <div className="flex items-center gap-2">
              ...
              <span>{"شرکت هواپیمایی با موفقیت اضافه شد"}</span>
            </div>,
          );

          router.push(
            routes.superadmin["transportation-options"].airplane.root,
          );
          router.refresh();
        }
      })
  ...
  return (
    <Dialog
      open={isAddPage}
      onOpenChange={() =>
        router.push(routes.superadmin["transportation-options"].airplane.root)
      }
    >
