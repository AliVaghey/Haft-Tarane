...
const AddForm = () => {
  ...
  const isAddPage = pathname.endsWith(
    routes.superadmin["transportation-options"].train.add,
  );
        ...
        if (response.status === 200) {
          toast.success(
            <div className="flex items-center gap-2">
              ...
              <span>{"شرکت حمل و نقل ریلی با موفقیت اضافه شد"}</span>
            </div>,
          );

          router.push(routes.superadmin["transportation-options"].train.root);
          router.refresh();
        }
      })
      .catch((error) => {
