...
const AddForm = () => {
  ...
  const isAddPage = pathname.endsWith(
    routes.superadmin.options.certificates.add,
  );
    ...
    await axios
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          router.push(routes.superadmin.options.certificates.root);
          router.refresh();
        }
      })
      .catch((error) => {
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="mr-4 text-right">افزودن مدرک</DialogTitle>
        </DialogHeader>
