...
const AddForm = () => {
  ...
  const isAddPage = pathname.endsWith(
    routes.superadmin.options["room-types"].add,
  );
        ...
        if (response.status === 200) {
          toast.success(
            <div className="flex items-center gap-2">
              ...
              <span>{"نوع اتاق با موفقیت اضافه شد"}</span>
            </div>,
          );

          router.push(routes.superadmin.options["room-types"].root);
          router.refresh();
        }
      })
      .catch((error) => {
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="mr-4 text-right">افزودن نوع اتاق</DialogTitle>
        </DialogHeader>
