...
const AddForm = () => {
  ...
  const isAddPage = pathname.endsWith(
    routes.superadmin.options["tour-styles"].add,
  );
        ...
        if (response.status === 200) {
          toast.success(
            <div className="flex items-center gap-2">
              ...
              <span>{"مدل تور با موفقیت اضافه شد"}</span>
            </div>,
          );

          router.push(routes.superadmin.options["tour-styles"].root);
          router.refresh();
        }
      })
      .catch((error) => {
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="mr-4 text-right">افزودن مدل تور</DialogTitle>
        </DialogHeader>
