...
const EditForm = ({ data }) => {
  ...
  const isAddPage = pathname.endsWith(routes.superadmin.admins.edit(data.id));
    ...
    await axios
      ...
      .then((response) => {
        if (response.status === 204) {
          ...
          router.push(routes.superadmin.admins.root);
          router.refresh();
        }
      })
      .catch((error) => {
  ...
  return (
    <Dialog
      open={isAddPage}
      onOpenChange={() => router.push(routes.superadmin.admins.root)}
    >
